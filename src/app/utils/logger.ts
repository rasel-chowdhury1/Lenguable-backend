...
export const logHttpRequests = (
  ...
  res.on("finish", () => {
    ...
    logger.info({
      message: `🖥️ IP: ${clientIp} 📅: ${new Date().toLocaleDateString("en-US", { weekday: "long" })} 🌐 Incoming Request: ${colorizeByMethod(req.method)} ${colorizeByStatusCode(res.statusCode)} ${magenta(req.originalUrl)} ⏱️ Response Time: ${yellowBright(`${responseTimeMs} ms`)}`,
      size: res.get("Content-Length") || 0,
    });
